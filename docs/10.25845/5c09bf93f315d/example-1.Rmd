...
This example assumes you have obtained an [AIMS Data Platform API Key](https://aims.github.io/data-platform/key-request).  Further details about key usage can be found [here](https://aims.github.io/data-platform-r/index#usage-of-aims-dataplatform-api-key)

To download and run this example interactively in *R Studio* click on `Code -> Download Rmd` in the top right corner of this document.
...
Now the results have been stored in the `results` object we can attempt to plot.
...
```{r}
plot(results$dataFrame$time, results$dataFrame$qc_value, type='l', xlab="Date", ylab="Air Temperature")
...
```{r}
plot(results$dataFrame$time, results$dataFrame$qc_value, type='l', xlab="Date", ylab="Air Temperature", main="Davies Reef Platform Air Temperature", col='green')
...
plot(results$dataFrame$time, results$dataFrame$qc_value, type='l', xlab="Date", ylab="Air Temperature", main="Air Temperature at AIMS Weather Stations", col='green')
