...
`dataaimsr` can be installed and loaded directly from [GitHub](https://github.com/aims/data-platform-r) using the following code:
...
```{r, message = FALSE}
davies <- aims_data_get(weather_doi,
                        api_key = my_api_key,
...
results <- plyr::llply(target_series, function(series_number, my_api_key, ...) {
  aims_data_get(weather_doi, api_key = my_api_key, 
